...
if [ ! -d "$SRC_DIR/klayout" ]; then
	mkdir "$SRC_DIR/klayout"
fi
cd "$SRC_DIR/klayout" || exit
sudo apt-get -y install qtbase5-dev qttools5-dev libqt5xmlpatterns5-dev qtmultimedia5-dev libqt5multimediawidgets5 and libqt5svg5-dev
sudo apt-get install libz-dev
sudo apt-get install libgit2-dev
sudo apt install gcc
sudo apt install g++
sudo apt-get install ruby-full
wget https://www.klayout.org/downloads/source/klayout-$KLAYOUT_VERSION.tar.gz
tar hzxvf klayout-$KLAYOUT_VERSION.tar.gz
cd klayout-$KLAYOUT_VERSION
./build.sh
sudo cp -r bin-release/ /usr/local/lib/klayout_$KLAYOUT_VERSION
sudo ln -s /usr/local/lib/klayout_$KLAYOUT_VERSION/klayout /usr/local/bin/klayout
sudo sh -c "echo /usr/local/lib/klayout_$KLAYOUT_VERSION >> /etc/ld.so.conf.d/libc.conf"
sudo ldconfig
sudo apt install cmake # for gf180
sudo apt install -y curl # for gf180
sudo apt-get install libcurl4-gnutls-dev # for gf180
sudo apt install qhull-bin # for gf180
sudo apt-get install libqhull* # for gf180
pip install docopt pandas gf180 # gdstk==0.9.40になってしまう。影響有り？
...
if [ ! -d "$SRC_DIR/ngspice" ]; then
	...
	./autogen.sh
	./configure --disable-debug --with-readline=yes --enable-openmp --enable-xspice
